...
title='Hi, Today_s weather is'
message='Thank you for reading'
notification.notify(title=title,message=message,app_icon=None,timeout=5,toast=False)
object_root = Tk()
object_root.geometry('600x600')
object_root['bg']='lavender'
object_root.title('Weather App')  # this is the title of the Weather Application
iconPhoto = PhotoImage(file='n.png')

object_root.iconphoto(False, iconPhoto)  # this is the icon of the Weather Application

headingLabel = Label(object_root, text="Today Weather", fg='purple', bg='lavender', font=" ArialBlack 23")
headingLabel.pack()

photo = PhotoImage(file='n.png')
photoLabel = Label(image=photo, width=100, height=100,bg='lavender')
photoLabel.pack()

searchCityLabel = Label(object_root, text="Search City ", fg='purple', bg='lavender', font=" ArialBlack 14 ")

searchCityLabel.pack()

canvas1 = Canvas(object_root, width=400, height=600,bg='lavender')
canvas1.pack()
...
def getWeather():
    ...
    labelCity = Label(object_root, text="Currently in " + city.upper(), fg='purple',bg='lavender', font="TimesNewRoman 15 ")
    canvas1.create_window(200, 180, window=labelCity)

    labelTemperature = Label(object_root, text="Temperature: " + temperature + ", " + details, fg='purple',bg='lavender',
                             font='TimesNewRoman 15')
    canvas1.create_window(200, 240, window=labelTemperature)

    labelPressure = Label(object_root, text="Pressure: " + pressure, fg='purple',bg='lavender', font="TimesNewRoman15 ")
    canvas1.create_window(200, 300, window=labelPressure)

    labelHumidity = Label(object_root, text="Humidity: " + humidity, fg='purple',bg='lavender', font="TimesNewRoman 15 ")
    canvas1.create_window(200, 360, window=labelHumidity)




button1 = Button(text='Search', command=getWeather, bg='white', fg='black')
